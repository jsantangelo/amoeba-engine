...
public final class MatrixHelper
{
	 ...
	 */
	public static float[] createMatrix(final Point translation, final Dimension scale, final float rotation)
	{
		final float[] matrix = new float[MATRIX_SIZE];

		Matrix.setIdentityM(matrix, 0);
		Matrix.translateM(matrix, 0, translation.getX(), translation.getY(), 0f);
		Matrix.scaleM(matrix, 0, scale.getWidth(), scale.getHeight(), 1.0f);
